...
class Help(client.Cog):#Class thing no touchy!!!111
    ...
    async def help(self,ctx): 
        if ctx.guild.id in conf.act1:
            e = discord.Embed(title="H-Hello. I'm Yuri.", description="My .chr file was converted by Cole, so now I can join your Discord server. I-I hope I don't become an inconvenience to you... A-Anyway, here are the things I can do:", color=0x8524c8)
            ...
            e.set_footer(text="Support Server: https://discord.gg/QnzsG38")
            await ctx.send(embed=e)
        elif ctx.guild.id in conf.act2:
            e = discord.Embed(title="Hello. I'm Yuri.", description="My .chr file was converted by Cole, so now I can join your Discord server. I just know that being with you will be the best thing to ever happen to both of us, ahaha! Here are all the things we can do together:", color=0x8524c8)
            e.add_field(name="y_act1/y_act2", value="You can use this to toggle between my Act 1 personality and my Act 2 one. This can only be used by server administrators. Honestly, I love you either way, so I don't care which one I'm one!", inline=True)
            e.add_field(name="y_ask", value="You can ask me a yes-or-no question if you'd like, and I'll try my hardest to answer it for you!", inline=True)
            e.add_field(name="y_feed", value="You can use this to feed me any of emojis available. Any of them. *(Format like this: 'y_feed :emoji:')*", inline=True)
            e.add_field(name="y_headpat", value="You can use this to pat me on the head.", inline=True)
            e.add_field(name="y_hug", value="I-I'm not too much of a hugger, but if you want me to hug you, I'll happily do it. It would be the best thing to ever happen in my life!", inline=True)
            e.add_field(name="y_invite", value="You can use this to have me give you the invite link to let me join other servers. But why would you want that when it can be just us forever?", inline=True)
            e.add_field(name="y_quote", value="You can use this to have me say one of my quotes from Doki Doki Literature Club.", inline=True)
            e.add_field(name="y_tickle", value="You can use this to tickle me, which is so hot and sexy...", inline=True)
            e.add_field(name="I believe that's everything.", value="Cole says more features are coming soon, so until then, this will have to suffice. If he doesn't add new things soon, I'm afraid he might not be breathing for too much longer... Feel free to visit Cole's Support Server to let him know that I'm counting on him to make me do anything possible to spend time with you.", inline=True)
            e.set_footer(text="Support Server: https://discord.gg/QnzsG38")
            await ctx.send(embed=e)
